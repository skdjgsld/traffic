...
# ---------- Yeni: consent butonuna tıklama ----------
def click_consent_button(page):
    """
    Eğer sayfada onclick attribute içinde consentGrantedAdStorage geçen bir button varsa tıkla.
    Ayrıca "Yes" metni içeren butonları dene.
    """
    try:
        # doğrudan onclick attribute arama
        btn = page.query_selector("button[onclick*='consentGrantedAdStorage']")
        if btn:
            try:
                logging.info("Consent button (onclick) bulundu, tıklanıyor.")
                btn.click(timeout=2000)
                time.sleep(random.uniform(0.3, 1.0))
                return True
            except Exception:
                pass
        # fallback: text ile arama (ör. "Yes")
        btn2 = page.query_selector("button:has-text('Yes')")
        if btn2:
            try:
                logging.info("Consent button (text 'Yes') bulundu, tıklanıyor.")
                btn2.click(timeout=2000)
                time.sleep(random.uniform(0.3, 1.0))
                return True
            except Exception:
                pass
...
# ---------- Yeni: üst menü metinlerini al / clipboard'a yazmayı dene ----------
def extract_nav_texts(page):
    """
    Üst menüdeki yazıları al (ör. logo text ve nav link metinleri).
    Log'a yaz ve mümkünse tarayıcı clipboard'una da kopyala (başarısız olursa logta kalır).
    """
    try:
        nav = page.query_selector("div.wrap.bar[role='navigation']")
        if not nav:
            # fallback daha genel: .wrap.bar
            nav = page.query_selector("div.wrap.bar")
        if nav:
            text = page.evaluate("(el) => el.innerText", nav)
            if text:
                text = text.strip()
                logging.info("Üst menü metni (kopyalandı/loglandı): %s", text.replace("\n", " | "))
                # attempt to copy to clipboard (may fail in headless / without permissions)
                try:
                    page.evaluate("navigator.clipboard.writeText(arguments[0])", text)
                    logging.info("Nav text copied to clipboard (browser clipboard API used).")
                except Exception as e:
                    logging.debug("Clipboard write failed (likely headless or permission): %s", e)
                return text
    except Exception as e:
        logging.debug("extract_nav_texts error: %s", e)
    return None

# ---------- Yeni: hemen-giris linkine tıklama ve popup yakalama ----------
def click_hemen_giris(page):
    """
    a#hemen-giris tıklansın. Eğer yeni pencere/popup açılırsa onun URL'sini logla ve kapat.
    """
    sel = "a#hemen-giris"
    try:
        el = page.query_selector(sel)
        if not el:
            logging.info("hemen-giris öğesi bulunamadı.")
            return False
        # Beklenen popup durumunu yakala
        try:
            with page.expect_popup() as popup_info:
                el.click(timeout=3000)
            popup = popup_info.value
            # popup açıldıysa yüklenmesini bekle
            try:
                popup.wait_for_load_state(timeout=5000)
            except Exception:
                pass
            try:
                logging.info("Popup açıldı. URL: %s", popup.url)
            except Exception:
                logging.info("Popup açıldı (URL alınamadı).")
            # kısa bekleme ve kapat
            ...
            # popup oluşmadıysa normal tıklama denenebilir (ve sayfanın START_URL'e geri dönmesi sağlanır)
            try:
                el.click(timeout=3000)
            except Exception:
                pass
            # eğer navigasyon olduysa START_URL'e geri dön
            try:
                if page.url != START_URL:
                    page.goto(START_URL, wait_until="domcontentloaded", timeout=15000)
            except Exception:
        ...
        logging.debug("click_hemen_giris error: %s", e)
    return False
...
def run_session(playwright, headless=False, max_pages=DEFAULT_MAX_PAGES):
    ...
    try:
        ...
        # Yeni: consent butonunu arayıp tıkla (varsa)
        try:
            clicked = click_consent_button(page)
            if clicked:
                logging.info("Consent button clicked successfully.")
        except Exception:
            pass
        ...
        # Yeni: Üst menü yazılarını al ve logla / kopyalamayı dene
        try:
            nav_text = extract_nav_texts(page)
            # ek opsiyon: log içinde ayrı olarak parse edebiliriz
            if nav_text:
                # örn: logda ana başlık ve link metinlerini ayrı gör
                logging.info("Nav text length: %d chars", len(nav_text))
        except Exception:
            pass

        # Yeni: 'hemen-giris' linkine tıkla (popup yönetimi ile)
        try:
            ok = click_hemen_giris(page)
            if ok:
                logging.info("Attempted to click #hemen-giris.")
        except Exception:
            pass
